...
public class Dao {
	...
	public String billcreate(Register robj) {
		try {
			query = "insert into bill(Cashiername,CustomerName,CustPhno,Date,Billno,productid,Itemname,Quantity,Price,PurchaseQuantity,Totalprice) values(?,?,?,?,?,?,?,?,?,?,?)";
			con = dobj.Dbconnect(); //db connector
			ps = con.prepareStatement(query); //query execution
